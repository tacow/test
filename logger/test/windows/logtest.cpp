...
DWORD WINAPI LogThread(void* ptr) {
...
int main() {
    ...
    HANDLE logThreads[LOG_THREADS];

    for(int i = 0; i < LOG_THREADS; ++i)
        logThreads[i] = CreateThread(NULL, 0, LogThread, (void*)i, 0, NULL);

	for(int i = 0; i < LOG_THREADS; ++i) {
        WaitForSingleObject(logThreads[i], INFINITE);
        CloseHandle(logThreads[i]);
    }
